...
class Analyzer(object): 
    ...
    def isAvNearNull(self):
        ...
        if exc == "EXC_BAD_ACCESS":
            ...
            try:
                ...
                print("WARNING: Malformed exception data %s %s" % ( code, extra))
    ...
    def getAccessType(self, insn):
        ...
        if operands.count(')') > 1:
            ...
            # rep/movsl     (%esi),(%edi)
            _, first, second, _ = self.R_ALNUMSPACE.sub('', operands).split()
    ...
    def getInsnRegisters(self, insn):
        ...
        if not insn or not insn.IsValid():
            return {}
        tokens = self.R_ALNUMSPACE.sub('', insn.operands).split()
        registers = self.getRegisters()
        involved = {}
        for t in tokens:
            if t in registers:
    ...
    def getExceptionData(self):
        ...
        desc = self.getStopDescription().translate('(),')
